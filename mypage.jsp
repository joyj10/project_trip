			...
			success:function(data){
				...
				if(arr.length == 0){
					...
					$.each(arr,function(i,m){
						...
						$(tr).append($("<td class=table_no></td>").html(m.b_no));
						$(tr).append($("<td></td>").html(m.b_region));
						$(tr).append($("<td class=table_date></td>").html(startday+" ~ "+endday));	
						$(tr).append($("<td></td>").html("<a href='detailBoard.do?b_no="+m.b_no+"'>"+m.b_title+"</a>"));			
						$(tr).append($("<td class=table_name></td>").html(m.m_name));
						$(tr).append($("<td class=table_hit></td>").html(m.b_hit));
						$("#tb").append(tr);
					});
				}
		}});
